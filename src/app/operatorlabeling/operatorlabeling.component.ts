...
@Component({
  selector: 'app-operatorlabeling',
  templateUrl: './operatorlabeling.component.html',
  styleUrls: ['./operatorlabeling.component.scss']
})
export class OperatorlabelingComponent{
 rows = [];
    selected: any[] = [];
  	closeResult: string;
modalReference: any;
...
constructor(private modalService: NgbModal,private http:HttpClient) {
    ...
    var config = {
     ...
     this.http.get('http://3.86.186.71:8080/api/operators/labeling', config)
     .subscribe(res=>{
        this.temp = [this.rows];
        this.rows = res['users'];
